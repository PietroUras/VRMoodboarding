            ...
            if (m_HandShapeDetected)
            {
                foreach (var condition in m_HandShape.fingerShapeConditions)
                {
                    foreach (var shapeCondition in condition.targets)
                    {
                        ...
                        // DEBUG LOG
                        Debug.Log($"Finger: {condition.fingerID} | Shape: {shapeCondition.shapeType} | Desired: {shapeCondition.desired} | Tolerance: (+{shapeCondition.upperTolerance} / -{shapeCondition.lowerTolerance})");
                    }
                }
            }
        }
